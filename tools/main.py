...
from bilibili.main import BiliBili
...
class BiliBiliButton(Button):
    def OnClick(self, event):
        try:
            ...
            while 1:
                time.sleep(0.2)
                if self.driver.current_url=='https://www.bilibili.com/':
                    get_cookies = self.driver.get_cookies()
                    cookie_str = ''
                    for s in get_cookies:
                        cookie_str = cookie_str + s['name'] + '=' + s['value'] + ';'
...
class CreateFrame(wx.Frame):
    ...
    def __init__(self, *args, **kw):
        ...
        # create a panel(面板) 当需要为应用程序创建窗口时使用框架；要向上放置其他部件时使用框架内的面板。
        self.pnl = wx.Panel(self)

        # create status bar(状态栏）
        statusBar = self.CreateStatusBar()
        statusBar.SetFieldsCount(2) #分两栏
        statusBar.SetStatusWidths([-5, -1]) #两栏宽度是5:1
        ...
        BiliBiliButton(self, self.pnl, Item(start_x, start_y+ystep*3,
            'BiliBili', 'resource/icon/bilibili.png'))
